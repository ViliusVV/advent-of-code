...
interface HandType {
    ...
    fun jokerizeHand(cards: List<Card>): List<Card> {
        val mostCommon = cards.groupBy { it }
            .filter { it.key != "J" }
            .maxByOrNull { it.value.size }
            ?.key ?: return cards

        return cards.map { if (it == "J") mostCommon else it }
    }
}
...
data object OnePair: HandType {
    override fun isHandType(cards: List<Card>): Boolean {
        return this.jokerizeHand(cards).groupBy { it }.filter { it.value.size == 2 }.count() == 1
    }
}

data object TwoPairs: HandType {
    override fun isHandType(cards: List<Card>): Boolean {
        return this.jokerizeHand(cards).groupBy { it }.filter { it.value.size == 2 }.count() == 2
    }
}

data object ThreeOfAKind: HandType {
    override fun isHandType(cards: List<Card>): Boolean {
        return this.jokerizeHand(cards).groupBy { it }.filter {it.value.size == 3}. count() == 1
    }
}

data object FullHouse: HandType {
    override fun isHandType(cards: List<Card>): Boolean {
        val grouped = this.jokerizeHand(cards).groupBy { it }
...
data object FourOfAKind: HandType {
    override fun isHandType(cards: List<Card>): Boolean {
        return this.jokerizeHand(cards).groupBy { it }.filter {it.value.size == 4}. count() == 1
    }
}

data object FiveOfAKind: HandType {
    override fun isHandType(cards: List<Card>): Boolean {
        return this.jokerizeHand(cards).groupBy { it }.filter {it.value.size == 5}. count() == 1
    }
}

val ALL_CARDS: List<Card> = listOf("J", "2", "3" , "4", "5", "6", "7", "8", "9", "10", "T", "Q", "K", "A")
